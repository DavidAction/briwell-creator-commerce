...
## 0.0.23 문서 텍스트 추출(P12) + 완성도 항목(P11) (2026-07-12) — 테스트 478 통과/26 스킵 (DB포함 504)

0.0.22에 이어 David 입력 불필요 항목 마감. 리뷰 P11·P12가 대상.

1. **P12 문서 텍스트 추출** (`app/partners/text_extraction.py`, 표준 라이브러리만):
   docx/pptx/hwpx/xlsx는 전부 ZIP+XML이라 **의존성 추가 없이** 본문 텍스트 추출
   (w:t/a:t/hp:t/sharedStrings, 슬라이드·섹션 번호순). 방어적 설계 — zip bomb 대비
   파트당·전체 용량 상한, DTD/ENTITY 포함 파트는 적대적 입력으로 간주하고 스킵
   (billion-laughs), 어떤 파싱 실패도 None(정직한 메타데이터-온리 폴백)으로 강등.
   연결: Anthropic 분류 경로(`_anthropic_content_parts`)와 Gemini 경로(`_upload_part`)
   모두 추출 텍스트를 인라인 — 기존 "파일명 기준 분류" 캐비앳이 "서버 추출 텍스트
   기준 분석(레이아웃·이미지 제외)"으로 격상. xlsx "Phase 2" 스킵도 해소.
   **`.hwp`(OLE)는 의도적으로 제외** — 적대적 입력에 수제 OLE 파서는 보안 부채이고
   검증된 파서는 우리 파이썬 버전 미지원. 기존 정직 캐비앳 유지, 도입 시 별도 결정.
2. **P11 완성도**: ①대시보드 "등록된 파트너" 조회 패널(목록 API는 있었으나 화면이
   없었음 — 행 선택 시 허브 링크 발급 폼에 파트너 ID 자동 채움) ②허브 초안 카드에
   완성도 구성요소 분해(기본 정보/전성분/사진/클레임/규제 — 무엇을 채우면 점수가
   오르는지) ③허브 재로딩 시 업로드 체크 선택 유지(state.picks). 페이지네이션은
   파일럿 규모(LIMIT 100–200)에서 불요 판단, SaaS 단계 항목으로 유지.
3. **테스트 격리 결함 발견·수정**: 전체 DB 스위트가 outreach dm_sent 테스트의
   `audit_event.persist` pending 잡을 공유 dev DB에 남기고, **다음** 실행의
   job_queue 테스트가 FIFO claim으로 이전 잡을 먼저 집어 실패(오늘 실DB 반복
   실행으로 처음 노출 — 이전에는 매번 새 DB라 은폐). 테스트를 자기 잡 id까지
   드레인하도록 수정, **연속 2회 실행 모두 504 통과**로 검증.
4. **검증**: 백엔드 478 통과/26 스킵(DB-off), RUN_DB_TESTS=1 **504 통과 ×2연속**,
   대시보드 스모크 통과(파트너 목록 어서션 추가), 실브라우저 — 파트너 목록
   로드·선택→폼 반영, 허브 구성요소 칩 렌더, 저장(재로딩) 후 체크 상태 유지 확인.

## 0.0.24 적대적 재검증 라운드 — 0.0.22~23 산출물 전수 리뷰·수정 (2026-07-12) — 테스트 480 통과/26 스킵 (DB포함 506)

David 지시("작업한 것 모두 비판적 검증·평가 후 최고 작업물로")에 따라 두 커밋
(52dd0cb·aa6bf0b)의 전 산출물을 적대적으로 재검토. 발견 7건 전부 수정·검증 완료.

**발견·수정 (심각도순):**
1. **퍼지 매칭 회귀** — CosIng 버킷화(첫 글자)가 첫 글자 오타를 못 잡게 만듦
   ("Xiacinamide"→Niacinamide ratio 0.909는 P3 이전엔 매칭). 큐레이션 시드는 항상
   전수 스캔하도록 수정(80여 개라 비용 0) + 회귀 테스트.
2. **DTD 가드 우회 가능성** — billion-laughs 가드가 앞 2KB만 검사해 주석으로
   프롤로그를 밀어내면 우회 가능했음. 파트 전체 스캔으로 변경(파트는 이미 용량 상한).
3. **링크 만료 미고지 UX 공백** — 토큰은 90일 만료인데 파트너는 만료일을 볼 수 없어
   "죽은 링크 서프라이즈"가 예정돼 있었음. `/partner-hub/me`가 `link_expires_at`을
   화이트리스트로 동봉, 허브 푸터에 "이 링크는 YYYY-MM-DD까지 유효합니다" 표시
   (실DB 왕복 스크립트에도 어서션 추가).
4. **팝업 차단 시 원본 보기 무반응** — 대시보드 원본 보기가 await 후 window.open이라
   유저 제스처 컨텍스트를 잃고 차단될 수 있음. 차단 감지 시 다운로드 앵커 폴백 +
   토스트 안내.
5. **동적 쓰기 버튼 칩 누락** — 주의 큐의 재분석 버튼(동적 생성)이 LIVE/PREVIEW 칩
   갱신 시점을 놓침. 렌더 후 updateWriteActionChips 호출.
6. **데모 픽스처 불일치** — 데모 completeness에 components가 없어 신기능(분해 칩)이
   데모에서 안 보였고, 추가하면서 점수 78 vs 구성요소 합 85 불일치도 발견 → 85로
   정합화. link_expires_at도 데모에 추가.
7. **RBAC 테스트 공백** — attention 큐·재분석 엔드포인트의 viewer 403 미고정 → 추가.

**검증:** 백엔드 480 통과/26 스킵(DB-off), RUN_DB_TESTS=1 **506 통과**, 대시보드
스모크 통과, 실DB 왕복 통과(만료 고지 포함), 실브라우저 — 라이브 모드 만료 문구·
데모 모드 분해 칩 렌더 확인.

**평가 (리뷰 v0 등급 대비, 자화자찬 금지 기준):** 파일럿 투입 준비도 C+ → **B+**.
리뷰의 "실파트너 투입 전 필수" 보안 3건(P1·P2·P6)과 데이터 1건(P3), 루프 4건
(P5·P7·P9·P10)이 코드·실DB·실브라우저 3중 검증으로 닫힘. A가 아닌 이유(정직):
①가치 증명 여전히 0% — 골든셋·정확도·절감시간 실측 전이고 라이브 AI 경로는 여전히
미실행(P4, 파넬 실자료 필요) ②알림 부재로 "폴링 노동" 미해소(P8, 발신 계정 필요)
③업로드 백업 없음(로컬 디스크 단일 사본) ④HWP 본문 추출 미지원(라이브러리 결정
필요). 전부 David 입력 대기 항목이며 HANDOFF에 기록됨.

## 0.0.18 최종 3테마 심화 + 수익루프·포털 섹션 + IR 보드 (2026-07-14)

David 픽 3종(1위 포토카드 팬덤 · 6위 플레이그라운드 · 8위 The Well)을 "함께 버는 구조"까지
보여주는 페이지로 심화. **브랜드 규칙 신설: briwell은 한 단어 — 자간 분리 표기 금지**
(위반 6곳 발견·수정: 칼마 계열의 자간 분리 로고).

1. **신규 섹션 2종 × 3페이지** (테마 네이티브 스킨): ① "Así ganamos juntas/juntos" — 수익
   순환 루프(서울→창작→판매→커미션) + Tu parte/Nuestra parte 분배 카드. 핵심 카피 =
   **"solo ganamos cuando tú ganas"**(가입비 0·크리에이터 과금 없음 → 인센티브 정렬).
   ② 포털 프리뷰 — 테마별 폰 목업($4,820 일러스트 잔액 + 코드 + 무브먼트, "pantalla
   ilustrativa" 표기) + 신뢰 포인트 3개. The Well은 중립형(nosotros/juntos), 팬덤·플레이
   그라운드는 creadoras 보이스(nosotras/juntas) — 페이지 내 일관성 검수 완료.
2. **IR 스크린 보드** `work/briwell_landing_page/ir/index.html` (KO, The Well, noindex):
   슬라이드 3장 — BM 순환(브랜드사→Briwell→크리에이터→소비자→정산, 수익 3카드 + 시스템
   게이트 배지), 시스템 화면(포털 폰 + 운영콘솔·정산 실화면), 소비자 접점 3종(라이브 iframe
   미리보기). 캡처 가이드·일러스트 수치 명시. 공유 패키지에 07_IR스크린보드.jpg 추가.
3. **검증**: 4페이지 × 1280/375 감사 오버플로 0·이미지 깨짐 0, 신규 섹션 육안 검수, 백엔드
   480 통과/26 스킵(타 세션에서 +102개 증가 확인). ⚠️ 홈페이지_순위별 PDF·ZIP 재생성은
   PDF가 뷰어에 잠겨 보류 — 뷰어 닫은 뒤 재생성 필요.
