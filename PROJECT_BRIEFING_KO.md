...
| 3 | **AI 자동 컨택 + 계약 관리** | 부분(DM 생성만·수동발송). 공식 레일(TikTok Shop Targeted Collaboration API, 일 1,000건 허용) 자동화로 확장 예정 |
| 4 | **Shopify 어필리에이트 + 자동 정산** (크리에이터 커머스) | 미구현(정산 레코드만 존재) |
| 5 | **배송 추적** | 미구현 |
| 6 | **자사 브랜드·제품·재고 인벤토리 관리 + AI 재주문** | 미구현(products 라우터만 존재) |

**아웃리치 정책 재조정**: 자동화 금지 게이트는 "오프플랫폼 비공식 DM"에만 유지.
TikTok Shop Affiliate API·Shopify Collabs 등 **플랫폼이 공식 제공하는 초대/협업 레일 경유 자동화는 허용**(ToS 적합).
